...
void CreateBins(vector<biner>& bins, particle_t* particles, int n)
{
...
int main( int argc, char **argv )
{
    ...
    CreateBins(particle_bins, particles, n);
    //
    ...
    for( int step = 0; step < NSTEPS; step++ )
    {
        ...
        for( int i = 0; i < binNum; i++ )
        {
          ...
          {
           biner& vec = particle_bins[i*binNum+j];
           ...
           for(int dx = -1; dx <= 1; dx++)
           {
            ...
            {
              if(i + dx >= 0 && i + dx < binNum && j + dy >= 0 && j+dy < binNum) // checks bins 1 - 8
              {
                biner& vec2 = particle_bins[(i+dx)*binNum+j+dy];
                for(int k = 0; k < vec.size(); k++)
        ...
        #pragma omp barrier  //wait for the master to finish mostly
    }
}
